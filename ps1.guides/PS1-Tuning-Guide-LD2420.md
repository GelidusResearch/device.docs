...
These two data elements of range (trip time) and presence (amplitude) are continuously processed. If a reflected chirp is detected, the processor activates a presence time window countdown timer, and the presence logic is set true both on the OT1 pin and via serial command data transmission. Any changes in the reflected chirp's amplitude or trip time that exceed the tuning thresholds during this window restart the presence timer, maintaining the presence true state. If the timer countdown expires with no detections, the presence state will be set to false.
